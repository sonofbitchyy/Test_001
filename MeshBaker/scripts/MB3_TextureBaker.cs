...
public class MB3_TextureBaker : MB3_MeshBakerRoot {	
    ...
    public List<GameObject> objectParent; //todo make this Renderer
    public Int64 objects_vertexCount = 0;
    public bool UpdateObjectBool = false;
    ...
    void OnDrawGizmos()
    {
        if (UpdateObjectBool)
        {
            UpdateObjectBool = false;
            if(objectParent.Count==0) 
            {
                Debug.LogError("please select'objectParent'");
                return;
            }
             objsToMesh.Clear();
             objects_vertexCount = 0;
            foreach (GameObject meshParent in objectParent)
	        {
		        MeshRenderer[] mrlist = meshParent.GetComponentsInChildren<MeshRenderer>();
                foreach(MeshRenderer mr in mrlist)
                {
                    objsToMesh.Add(mr.gameObject);
                    Mesh temp = mr.gameObject.GetComponent<MeshFilter>().sharedMesh;
                    objects_vertexCount += temp.vertexCount;

                }
	        }
        }
        if(objsToMesh.Count!=objectCount)
        {

            objectCount = objsToMesh.Count;
            objects_vertexCount = 0;
            foreach (GameObject mr in objsToMesh)
            {
                if(mr!=null)
                {
                    Mesh temp = mr.GetComponent<MeshFilter>().sharedMesh;
                    objects_vertexCount += temp.vertexCount;
                }
